...
map<int,shared_ptr<variable>> Cardinality;//cardinality of each color
map<int,map<int,shared_ptr<variable>>> difference;
shared_ptr<variable> Finalvalue;
...
//set the cardinality of each color
void set_cardinality(){
	for(int i=0;i<25;i++){
		Cardinality[i]=make_shared<variable>();
		Cardinality[i]->id=vvar.size();
		Cardinality[i]->type=1;
		Cardinality[i]->upbound=25;
		Cardinality[i]->lobound=0;
		vvar.push_back(Cardinality[i]);
	}
}
//cardinality= sum of colormap[i][j]
void set_constraint_group4(){
	for(int i=0;i<25;i++){
		sum_constr++;
		constr_sense_map[sum_constr]=0;
		constr_rightside_map[sum_constr]=0;
	        constr_variable_map[sum_constr].insert(Cardinality[i]);
        	Cardinality[i]->constr_involved.insert(sum_constr);
        	Cardinality[i]->constr_coeff[sum_constr]=1;
		for(auto &j:vecnode){
			constr_variable_map[sum_constr].insert(color_map[i][j->id()]);
			color_map[i][j->id()]->constr_involved.insert(sum_constr);
			color_map[i][j->id()]->constr_coeff[sum_constr]=-1;
...
void set_difference(){
	for(int i=0;i<25;i++){
		for(int j=0;j<25;j++){
			difference[i][j]=make_shared<variable>();
			difference[i][j]->id=vvar.size();
			difference[i][j]->type=2;
			difference[i][j]->upbound=25;
			difference[i][j]->lobound=-25;
			vvar.push_back(difference[i][j]);
		}
	}
}
void set_finalvalue(){
		Finalvalue=make_shared<variable>();
		Finalvalue->id=vvar.size();
		Finalvalue->type=2;
		Finalvalue->upbound=25;
		Finalvalue->lobound=-25;
		vvar.push_back(Finalvalue);
}
//absolutvalue=cardi[i]-cardi[j];
void set_constraint_group5(){
	for(int i=0;i<25;i++){
		for(int j=0;j<25;j++){
			...
			constr_sense_map[sum_constr]=0;
			constr_rightside_map[sum_constr]=0;
			constr_variable_map[sum_constr].insert(difference[i][j]);
			difference[i][j]->constr_involved.insert(sum_constr);
			difference[i][j]->constr_coeff[sum_constr]=1;
			constr_variable_map[sum_constr].insert(Cardinality[i]);
			Cardinality[i]->constr_involved.insert(sum_constr);
			Cardinality[i]->constr_coeff[sum_constr]=-1;	
			constr_variable_map[sum_constr].insert(Cardinality[j]);
			Cardinality[j]->constr_involved.insert(sum_constr);
			Cardinality[j]->constr_coeff[sum_constr]=1;
		}
	}
}
void set_constraint_group6(){
	for(int i=0;i<25;i++){
		for(int j=0;j<25;j++){
			...
			constr_sense_map[sum_constr]=1;
			constr_rightside_map[sum_constr]=0;
			constr_variable_map[sum_constr].insert(Finalvalue);
			Finalvalue->constr_involved.insert(sum_constr);
			Finalvalue->constr_coeff[sum_constr]=1;
			constr_variable_map[sum_constr].insert(difference[i][j]);
			difference[i][j]->constr_involved.insert(sum_constr);
			difference[i][j]->constr_coeff[sum_constr]=-1;
		}
...
int main(int argc, char * argv[]){
	...
	objcons=sum_constr;
	funcGurobi(30,0,1);
/*	cout<<Colornumber->int_value<<" "<<Finalvalue->int_value<<"  "<<endl;
	cout<<Colornumber->int_value+Finalvalue->int_value<<"  "<<endl;*/
	for(int i=0;i<25;i++)
	{
		...
		for(auto &j:vecnode)
		{		
			cout<<color_map[i][j->id()]->int_value<<", ";

		}
	cout<<"}"<<endl;
	}
	cout<<Colornumber->int_value<<endl;
}	
